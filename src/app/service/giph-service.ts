...
export class GiphService {
  ...
  searchGifs(query:string, limit: number = 25, offset: number = 0, rating: string ='g') : Observable<any>{
    const params = new HttpParams().set('apiKey', this.apiKey).set('q',query).set('limit', limit.toString()).set('offset',offset.toString()).set('rating',rating).set('bundle','messaging_non_clips');
